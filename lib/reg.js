...
class XuetangX {
  ...
  async getPlayurl(ccid, quality) {
    const res = await this.axios.get(`/api/v1/lms/service/playurl/${ccid}/`, {
      params: {
        appid: 10000,
      },
    });
    const sources = res && res.data && res.data.data && res.data.data.sources;
    if (quality) {
      return sources && sources.quality20[0];
    }
    return sources && sources.quality10[0];
  }
