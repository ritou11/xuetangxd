...
class XuetangX {
  ...
  async getVideoCcid(leafInfoID, cid, sign) {
    const res = await this.getLeafInfo(leafInfoID, cid, sign);
    const media = res && res.content_info && res.content_info.media;
    return media && media.ccid;
  }

  async getVideoLink(leafInfoID, cid, sign) {
    const ccid = await this.getVideoCcid(leafInfoID, cid, sign);
    if (!ccid) return null;
